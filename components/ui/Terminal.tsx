...
export function Terminal() {
  ...
  const [history, setHistory] = useState<HistoryItem[]>([
    {
      ...
      output: (
        <div className="space-y-1 text-xs">
          <p className="text-[#00a86b] dark:text-[#00f5a0] font-semibold">
            HENOCK-OS v4.2.0-LTS [Quantum Terminal Active]
          </p>
          <p className="text-slate-600 dark:text-text-secondary">
            Welcome, visitor. Type <span className="text-[#0099bb] dark:text-[#00d4ff] font-bold">help</span> or click a quick-action command below.
          </p>
        </div>
      ),
    },
        ...
        output = (
          <div className="space-y-1.5 text-xs text-slate-600 dark:text-text-secondary">
            ...
            <div className="grid grid-cols-2 gap-x-4 gap-y-1">
              ...
              <div><span className="text-emerald-600 dark:text-emerald-400">contact</span> - Get in touch</div>
              <div><span className="text-cyan-600 dark:text-cyan-400">hire</span> - Employment availability</div>
              <div><span className="text-red-500 dark:text-red-400">clear</span> - Clear terminal window</div>
            </div>
          </div>
        );
        break;
      ...
      default:
        output = (
          <p className="text-xs text-red-500 dark:text-red-400">
            command not found: {cleanCmd}. Type <span className="text-[#00a86b] dark:text-[#00f5a0] underline cursor-pointer" onClick={() => executeCommand("help")}>help</span> to see valid commands.
          </p>
        );
        break;
    }
  ...
  return (
    <div className="rounded-2xl bg-white dark:bg-[#0d0d1f]/90 border border-slate-200 dark:border-white/10 overflow-hidden shadow-2xl flex flex-col font-mono text-sm max-w-full transition-colors duration-300">
      {/* Terminal Title Bar */}
      <div className="px-4 py-3 bg-slate-100/90 dark:bg-black/40 border-b border-slate-200 dark:border-white/10 flex items-center justify-between transition-colors duration-300">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs text-slate-500 dark:text-text-muted ml-2 font-mono flex items-center gap-1.5">
            <TerminalIcon size={12} className="text-[#00a86b] dark:text-[#00f5a0]" />
            henock@terminal: ~ (zsh)
          </span>
        </div>
      ...
      <div 
        ref={terminalBodyRef}
        className="p-4 sm:p-5 h-64 overflow-y-auto space-y-3 bg-slate-50/95 dark:bg-[#050510]/80 select-text transition-colors duration-300"
        onClick={() => inputRef.current?.focus()}
      >
        {history.map((item, index) => (
          <div key={index} className="space-y-1">
            <div className="flex items-center gap-2 text-xs">
              <span className="text-[#00a86b] dark:text-[#00f5a0]">➜</span>
              <span className="text-[#0099bb] dark:text-[#00d4ff]">~</span>
              <span className="text-slate-900 dark:text-white font-medium">{item.command}</span>
            </div>
      ...
      {/* Quick Action Chips */}
      <div className="px-4 py-2 bg-slate-100/80 dark:bg-black/50 border-t border-slate-200 dark:border-white/5 flex flex-wrap gap-1.5 items-center transition-colors duration-300">
        <span className="text-[10px] text-slate-500 dark:text-text-muted uppercase tracking-wider mr-1">Quick:</span>
        {["help", "bio", "skills", "projects", "contact", "clear"].map((cmd) => (
          <button
            key={cmd}
            type="button"
            onClick={() => handleChipClick(cmd)}
            className="text-[11px] px-2.5 py-0.5 rounded-md bg-white dark:bg-white/5 hover:bg-emerald-50 dark:hover:bg-[#00f5a0]/15 hover:text-[#00a86b] dark:hover:text-[#00f5a0] border border-slate-200 dark:border-white/10 text-slate-600 dark:text-text-secondary transition-colors shadow-sm dark:shadow-none cursor-pointer"
          >
            {cmd}
          </button>
        ))}
      ...
      {/* Terminal Input Bar */}
      <form onSubmit={handleSubmit} className="px-4 py-3 bg-white dark:bg-black/60 border-t border-slate-200 dark:border-white/10 flex items-center gap-2 transition-colors duration-300">
        <span className="text-[#00a86b] dark:text-[#00f5a0] text-sm font-bold">➜</span>
        <span className="text-[#0099bb] dark:text-[#00d4ff] text-xs font-semibold">~</span>
        <input
          ref={inputRef}
          type="text"
          value={input}
          ...
          className="flex-1 bg-transparent border-none text-slate-900 dark:text-white text-xs font-mono focus:outline-none placeholder:text-slate-400 dark:placeholder-text-muted/60"
        />
        <button
          type="submit"
          className="text-slate-400 hover:text-[#00a86b] dark:text-text-muted dark:hover:text-[#00f5a0] transition-colors p-1 cursor-pointer"
